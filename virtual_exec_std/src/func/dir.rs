...
fn dir_sync<'a>(value: Object<'a>, Alloc(alloc): MemoryAllocator<'a>) -> Result<Collection<'a>, Error> {
    let mem_use = value.read_arc_safe().iter().map(|(k,_)| k.len()).sum::<usize>() + value.read_arc_safe().len() * 8;
    alloc.lock_arc_safe().check_alloc_err(mem_use)?;
    
    let dir: Vec<String> = value.read_arc_safe().iter().map(|(k, _)| k.clone()).collect();
